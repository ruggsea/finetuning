...
def main():
    ...
    OUTPUT_DIR = "llama70b-cot-wsdm"
    HUB_MODEL_ID = "ruggsea/Llama70B-CoT-WSDM"
    ...
    try:
        ...
        # LoRA configuration - using slightly larger rank for 70B model
        peft_config = LoraConfig(
            lora_alpha=16,
            lora_dropout=0.1,
            r=16,  # Increased from 8 for 70B model
            bias="none",
            task_type="CAUSAL_LM",
            target_modules=[
                "q_proj",
        ...
        # Training arguments - adjusted for larger model
        training_args = SFTConfig(
            ...
            do_eval=True,
            report_to="wandb",
            run_name=f"llama70b-cot-{datetime.now().strftime('%Y-%m-%d-%H-%M')}",
            warmup_ratio=0.03,
